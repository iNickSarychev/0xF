...
class LLMProcessor:
    ...
    async def process_news_batch(
        ...
        prompt = f"""Ты — профессиональный переводчик и футуролог.

ЗАДАНИЕ:
Ниже список из {len(news_list)} новостей. 
1. Выбери ОДНУ самую важную новость (согласно приоритету ниже).
2. Сделай её качественный перевод на русский язык.

ПРИОРИТЕТ ВЫБОРА:
1. Прорывные технологии («Future Tech»), которые перевернут мир (футуризм, кванты, биохакинг, новая энергия).
2. Будущее цивилизации (новые горизонты, колонизация космоса, радикальное продление жизни).
3. Важные и мощные релизы новых AI-моделей.
4. Прочие важные технологические тренды.

ПРАВИЛА ДЛИНЫ ТЕКСТА (ВАЖНО):
- Если новость про «Будущее» или «Прорывные открытия» — пиши ПОДРОБНО и ГЛУБОКО (объём от 1000 до 4000 символов).
- Если это обычная новость — пиши ЛАКОНИЧНО (объём от 200 до 800 символов, цель 400).
...
[Текст перевода и анализа, максимально раскрывающий суть]

ПРАВИЛА ОФОРМЛЕНИЯ:
- Пиши ТОЛЬКО на русском языке.
- Первая строка ответа ОБЯЗАТЕЛЬНО: НОМЕР: N.
- Используй <b>жирный</b> для выделения. НИКАКОГО Markdown (##, **, #).
- Для лонгридов (тем будущего) используй абзацы для лёгкого чтения.
- Не теряй смысл и важные цифры/детали.
